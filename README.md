...
><br>
>
>The project aims to assist visually impaired individuals in distinguishing 'everyday objects' like containers and cosmetic products. We've used `QR codes` for information storage, recognized by the camera, and incorporated `Braille output` for tactile identification. The device is `voice-operated` and designed with `specific features` to cater to the needs of visually impaired users.
<br>
<br>
<br>
